...
<h1 align="center">이런 X친 파이썬! 🐍</h1>
<p align="center">파이썬의 놀라운 snippet과 알려지지 않은 기능에 대하여.</p>

[![WTFPL 2.0][license-image]][license-url]

심미적으로 아름다운 인터프리터 기반의 개발 언어인 파이썬은 개발자의 편의성을 위해서 우리에게 많은 기능을 제공합니다. 하지만 가끔은 한눈에 알아보기 힘든 파이썬 코드도 존재하죠.

바로 이곳에, 여러분을 위한, 조금은 괴짜스럽고 비 직관적인, 그리고 잘 알려지지 않은 기능들이 있습니다. 어떤 것들이 있는지 한번 확인해보세요!

아래에 중 몇몇은 '이런 X친'이라는 말이 나오지 않는 예시일 수도 있습니다만, 여러분이 몰랐던 파이썬의 숨겨진 부분을 알려줄 것입니다. 개발 언어의 내부 구조에 대해서 재밌게 배울 수 있는 절호의 기회죠!

만약 여러분이 파이썬 개발자라면, 스스로를 위한 '코드를 한눈에 알아보기 문제'로 여겨보는 것도 재미있을 겁니다. 이미 익숙한 예시가 있다면, 여러분이 파이썬을 처음 배웠던 때로 돌아가 그 때의 기억을 되살려보는 것도 재미있겠죠:sweat_smile:

PS: 이전 버전을 읽으셨다구요? 새로운 변경점을 확인해보세요! [원문 변경점 보기](https://github.com/satwikkansal/wtfpython/releases/).
